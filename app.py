...
from soveil.data import DAYS, DEFAULT_INITIAL_LOSS, SITES, get_forecast, inject_demo_storm
...
with st.sidebar:
    ...
    with st.expander("Safety gate"):
        ...
        demo_storm = st.checkbox("Guarantee a storm-approval scenario", value=True,
                                 help="Injects one forecast spike so the human-approval demo is always ready to show, "
                                      "regardless of what the real/simulated forecast looks like this week. "
                                      "Turn off to see the unmodified forecast.")
...
if demo_storm:
    forecasts = inject_demo_storm(forecasts, sites[0], day_idx=2, storm_pm10=storm_pm10)
MW = SITES[sites[0]]["mw"]
...
if schedule is None:
    ...
    st.stop()
if demo_storm:
    # Force this exact cell to be a scheduled clean so the approval card is guaranteed to
    # appear, rather than depending on which day the optimizer happens to pick around the spike.
    schedule = {k: v.copy() for k, v in schedule.items()}
    schedule[sites[0]][2] = True
...
ui.hero(headline, f"Dust and rain forecasts decide when each of the {len(sites)} pilot sites gets cleaned")
    ...
    with st.container(border=True):
        ...
        for s, dt, lab in pending:
            ...
            if key in st.session_state:
                ...
                if st.button("↺ Reset this decision", key=key + "reset"):
